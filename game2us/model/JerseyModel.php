 ...
 * Time: 14:12
 */
define("JERSEY_MODEL_DEFAULT", 1);


class JerseyModel extends MysqlDB
{
    ...
    function init($_primaryValue)
    {
        if (empty($_primaryValue)) {
            printError("JerseyModel primaryValue is " . $_primaryValue);
            return false;
        }
...
//        $this->data = $this->getModelData(DATA_BASE_JERSEY, "userid", $userId);
        //$this->data($this->getModelData(DATA_BASE_JERSEY, "userid", $_primaryValue));
        $this->fillData();
        return true;
    }
...
class JerseyModelMgr
{
    ...
    private $_fieldList = array(
        JERSEY_MODEL_DEFAULT => array('userId'),
    );
    ...
    static function Instance()
    {
        ...
        {
            self::$_instance = new JerseyModelMgr();
    ...
    private function init()
    {
        ...
        if($_db)
        {
            $_db->TableName(DATA_BASE_JERSEY);
            $_db->PK(DATA_BASE_JERSEY);//设置主键(需要修改)DATA_BASE_CARD_RECOMMEND->DATA_BASE_CARD_RECOMMEND_PRI
            $_db->FieldList(eval(DATA_BASE_JERSEY_FIELD));//设置字段列表
            $_db->registerQueryField($this->_fieldList);//注册查询字段
            $_db->TableEnum(TABLE_JERSEY);
